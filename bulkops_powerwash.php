...
<head>
    ...
    <title>Bulk Operations: Remote Powerwash</title>
    <style>
    form {
        margin-bottom: 20px;
    }
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($dataType) && !empty($inputData)) {
            ...
            $commands = array_map(function($data) use ($dataType) {
                switch ($dataType) {
                    case "device_id":
                        return "gam cros " . $data . " issuecommand command remote_powerwash doit";
                    case "serial_number":
                        return "gam cros_sn " . $data . " issuecommand command remote_powerwash doit";
                    case "asset_id":
                        return "gam cros_query asset_id:" . $data . " issuecommand command remote_powerwash doit";
                    default:
