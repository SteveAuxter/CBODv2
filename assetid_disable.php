        ...
        if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['search_term'])) {
            ...
            if (empty($search_term)) {
                ...
                if ($counter === 0) {
                    ...
                    $command1 = sprintf("$GAMpath info cros query asset_id:%s status", $search_term);
                    exec($command1, $infoStatus);
                    $gamStatus = trim(substr($infoStatus[1],10));

                    if ($gamStatus == 'ACTIVE') {
                        $command2 = sprintf("$GAMpath update cros query \"asset_id:%s status:ACTIVE\" action disable", $search_term);
                        $sqlCommand = "UPDATE devices SET status = 'DISABLED' WHERE annotatedAssetId LIKE :search_term";
                        echo "<h3>Current status is ACTIVE - sending DISABLE action to Asset ID <font color='#008CBA'>$search_term</font>. Here's what happened:</h3>";
                    } elseif ($gamStatus == 'DISABLED') {
                        $command2 = sprintf("$GAMpath update cros query \"asset_id:%s status:DISABLED\" action reenable", $search_term);
                        $sqlCommand = "UPDATE devices SET status = 'ACTIVE' WHERE annotatedAssetId LIKE :search_term";
                        echo "<h3>Current status is DISABLED - sending REENABLE action to Asset ID <font color='#008CBA'>$search_term</font>. Here's what happened:</h3>";
                    } else {
