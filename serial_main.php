    ...
    <?php try {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['search_term'])) {
            ...
            if (empty($search_term)) {
                ...
                $stmt = $db->prepare("SELECT * FROM devices WHERE serialNumber LIKE :search_term");
                ...
                if ($counter === 0) {
                    ...
                    echo "<hr>";
                    echo "<h3>Asking Google about device with Serial Number <font color='#008CBA'>$search_term</font>. Here's what I found:</h3>";

                    // GAM Script: Proceed to query additional information from Google
                    $command1 = sprintf("$GAMpath print cros fields annotatedAssetId,annotatedLocation,annotatedUser,bootMode,deviceLicenseType,firmwareVersion,firstEnrollmentTime,lastEnrollmentTime,lastKnownNetwork,lastSync,macAddress,model,notes,orgUnitPath,osVersion,platformVersion,serialNumber,status query id:%s", $search_term);
                    exec($command1, $infoBasic);

                    $command2 = sprintf("$GAMpath print crosactivity query id:%s users", $search_term);
